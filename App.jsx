...
function App() {
  ...
  const handleCheckoutWithEmail = async () => {
    ...
    try {
      const lineItems = cart.map(item => ({
        name: item.nameWithSize || item.name,
        price: item.price,
        image: item.image,
        size: item.size
      }));
      ...
      const response = await fetch('/api/create-checkout-session', {
        ...
        body: JSON.stringify({ items: lineItems, email: customerEmail }),
      });
  ...
  if (showEmailModal) {
    return (
      <div style={{ backgroundColor: 'white', minHeight: '100vh', fontFamily: 'system-ui, -apple-system, sans-serif', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ backgroundColor: 'white', borderRadius: '24px', padding: '40px', maxWidth: '400px', width: '90%', boxShadow: '0 20px 40px rgba(0,0,0,0.1)', textAlign: 'center' }}>
          <div style={{ fontSize: '48px', marginBottom: '20px' }}>📧</div>
          <h2 style={{ fontSize: '24px', fontWeight: 'bold', marginBottom: '16px' }}>Enter Your Email</h2>
          <p style={{ color: '#666', marginBottom: '24px' }}>We'll send your order confirmation and tracking information to this email.</p>
          <input
            type="email"
